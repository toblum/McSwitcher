...
void loopWiFiManager() {
  ...
  // additional fields
  WiFiManagerParameter p_mqttHost("mqttHost", "MQTT host", config.mqttHost, STRUCT_CHAR_ARRAY_SIZE);
  WiFiManagerParameter p_mqttPort("mqttPort", "MQTT port", config.mqttPort, STRUCT_CHAR_ARRAY_SIZE);
  WiFiManagerParameter p_mqttTopic("mqttTopic", "MQTT topic", config.mqttTopic, STRUCT_CHAR_ARRAY_SIZE);
  WiFiManagerParameter p_pinReceiver("pinReceiver", "GPIO pin receiver", config.pinReceiver, STRUCT_CHAR_ARRAY_SIZE);
  WiFiManagerParameter p_pinSender("pinSender", "GPIO pin sender", config.pinSender, STRUCT_CHAR_ARRAY_SIZE);
 ...
 */
  ...
  wifiManager.addParameter(&p_mqttHost);
  wifiManager.addParameter(&p_mqttPort);
  wifiManager.addParameter(&p_mqttTopic);
  wifiManager.addParameter(&p_pinReceiver);
  wifiManager.addParameter(&p_pinSender);
  ...
  //additional fields
  strcpy(config.mqttHost, p_mqttHost.getValue());
  strcpy(config.mqttPort, p_mqttPort.getValue());
  strcpy(config.mqttTopic, p_mqttTopic.getValue());
  strcpy(config.pinReceiver, p_pinReceiver.getValue());
  strcpy(config.pinSender, p_pinSender.getValue());
 ...
 */
            ...
            strcpy(config.mqttPort, json["mqttPort"]);
            strcpy(config.mqttTopic, json["mqttTopic"]);
            strcpy(config.pinReceiver, json["pinReceiver"]);
            strcpy(config.pinSender, json["pinSender"]);
